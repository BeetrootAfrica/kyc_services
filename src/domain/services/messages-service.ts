...
export class MessagesService {
    ...
    async conversationsWithUnreceivedMessages(loggedUserId:number,) : Promise<number[]> {
        const rows = await this._messagesTypeormRepo.createQueryBuilder("message")
            ...
            .where(`${("message.receiverUserId" as keyof TextMessageEntity)} = :receiverUserId`, {receiverUserId: loggedUserId})
            .where(`${("message.receivedAt" as keyof TextMessageEntity)} is null`)
